...
import { clear, Coordinates, Dimensions, logInfo, operate, parseNumeric, stringify, unsubscription } from '../../../../../utils/utils';
...
import { MAX_CURRENT_ELEMENTS_PER_CARD_FACE } from '../../../card-editor/constants/card-editor.constants';
...
export class CardFaceElementService {
  private readonly cardFaceElementApiService: CardFaceElementApiService = inject<CardFaceElementApiService>(CardFaceElementApiService);
  ...
  public swapZIndex(cardFaceElement: string | CardFaceElementPerCardFace, cardFaceElementsPerCardFace: CardFaceElementPerCardFace[], direction: 'forward' | 'backward'): string {
    if (!cardFaceElementsPerCardFace) throw new Error("No card face elements per card face to grab max index");
    ...
    if (!current.cardFaceElement.style?.zIndex) throw new Error("Card face element has no style to add Z index to");

    let filtered: CardFaceElementPerCardFace[] = cardFaceElementsPerCardFace.filter((cardFaceElementPerCardFace: CardFaceElementPerCardFace) => {
      let za: number = parseInt(cardFaceElementPerCardFace.cardFaceElement.style?.zIndex ?? "1") || 0;
      let target: number = parseInt(current.cardFaceElement.style?.zIndex ?? '1') || 0;

      return (direction === 'forward') ? za > target : za < target;
    });

    let swap: CardFaceElementPerCardFace = filtered.reduce((a: CardFaceElementPerCardFace, b: CardFaceElementPerCardFace) => {
      let za: number = parseInt(a.cardFaceElement.style?.zIndex ?? "1") || 0;
      let zb: number = parseInt(b.cardFaceElement.style?.zIndex ?? "1") || 0;
      let target: number = parseInt(current.cardFaceElement.style?.zIndex ?? '1') || 0;

      return Math.abs(zb - target) < Math.abs(za - target) ? b : a
    });

     if (!current.cardFaceElement.style?.zIndex || !swap.cardFaceElement.style?.zIndex)
      throw new Error("Card face element has no style to add Z index to");

    [current.cardFaceElement.style.zIndex, swap.cardFaceElement.style.zIndex] = [swap.cardFaceElement.style.zIndex, current.cardFaceElement.style.zIndex];

    return current.cardFaceElement.style.zIndex;
  }

  public setLayer(cardFaceElementPerCardFace: CardFaceElementPerCardFace, cardFaceElementsPerCardFace: CardFaceElementPerCardFace[], operation: 'front' | 'back' | 'forward' | 'backward'): string;
  public setLayer(cardFaceElementId: string, cardFaceElementsPerCardFace: CardFaceElementPerCardFace[], operation: 'front' | 'back' | 'forward' | 'backward'): string;
  public setLayer(cardFaceElement: string | CardFaceElementPerCardFace, cardFaceElementsPerCardFace: CardFaceElementPerCardFace[], operation: 'front' | 'back' | 'forward' | 'backward'): string {
    let zIndex: string = "-1";
    ...
    switch (operation) {
      ...
      default:
        zIndex = this.swapZIndex(cardFaceElement, cardFaceElementsPerCardFace, operation);
    }
  ...
  public canAddCardFaceElementPerCardFace(amt: number): boolean {
    return amt < MAX_CURRENT_ELEMENTS_PER_CARD_FACE;
  }
  ...
  public createCardFaceElementPerCardFace(
    ...
    if (!this.canAddCardFaceElementPerCardFace(cardFaceElementsPerCardFace.length)) {
