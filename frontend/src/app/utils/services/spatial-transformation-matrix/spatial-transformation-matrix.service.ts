...
export class SpatialTransformationMatrixService {
  ...
  private getTransformation(transformation: 'translation' | 'rotation' | 'scale', coordinate: Coordinates): Coordinates {
    switch (transformation) {
      case 'translation': {
        return {
          x: (coordinate.x + this.IDENTITY_MATRIX[0][2]),
          y: (coordinate.y + this.IDENTITY_MATRIX[1][2])
        }
      }
      case 'rotation': {
        return {
  ...
  private setTranslationMatrix(delta: Coordinates): void {
    this.IDENTITY_MATRIX[0][2] = this.IDENTITY_MATRIX[0][2] + delta.x;
    this.IDENTITY_MATRIX[1][2] = this.IDENTITY_MATRIX[1][2] + delta.y;
  }
